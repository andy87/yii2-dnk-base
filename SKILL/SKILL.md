...
description: >-
  ...
  composer-пакет andy87/yii2-dnk или классы пакета yii2dnk Base.
---
...
Если в проекте уже есть локальные `Base*` классы с тем же назначением, не дублируй их. Предложи миграцию на `andy87/yii2-dnk` или явно зафиксируй, что проект пока остается на локальной реализации.

## 1.1. Ограничения runtime-пакета

`andy87/yii2-dnk` — минимальный DNK base layer, а не полноценный framework.
...
Сигнатуры ниже синхронизированы с runtime-пакетом `github/andy87/yii2-dnk/src` на момент проверки. При изменении исходников пакета сначала сверяй эти сигнатуры с текущим кодом.
...
- локальные копии Base-классов из `andy87/yii2-dnk`;
...
```bash
php /path/to/yii2-dnk-flow/scripts/generate-dnk-templates.php \
...
```bash
php /path/to/yii2-dnk-flow/scripts/sync-cursor-skill.php
```
